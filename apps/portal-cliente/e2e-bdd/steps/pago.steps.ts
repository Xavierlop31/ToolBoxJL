...
When('elijo pagar con {string}', async ({ page }, metodoLabel: string) => {
  ...
  await page.route(`**/orders/${ORDER_ID}/pay`, async (route) => {
    await route.fulfill({
      status: 201,
      contentType: 'application/json',
      body: JSON.stringify({
        ...
        estado,
        monto: 165000,
        wompi_transaction_id: metodo === 'contra_entrega' ? null : 'wompi-tx-001',
      }),
    });
  });
...
Given('que soy un Cliente pagando con tarjeta y mi orden requiere depósito de garantía', async ({ page }) => {
  await page.route(`**/catalog/models/${MODEL_ID}`, async (route) => {
    await route.fulfill({
      status: 200,
      contentType: 'application/json',
  ...
  await page.route(`**/orders/${ORDER_ID}/pay`, async (route) => {
    await route.fulfill({
      ...
      body: JSON.stringify({
        ...
        estado: 'hold',
        monto: 50000,
        wompi_transaction_id: 'wompi-tx-002',
      }),
    });
  });
...
Given('que soy un Cliente pagando con PSE o contra entrega y mi orden requiere depósito de garantía', async ({ page }) => {
  await page.route(`**/catalog/models/${MODEL_ID}`, async (route) => {
    await route.fulfill({
      status: 200,
      contentType: 'application/json',
  ...
  await page.route(`**/orders/${ORDER_ID}/pay`, async (route) => {
    await route.fulfill({
      ...
      body: JSON.stringify({
        ...
        estado: 'capturado',
        monto: 50000,
        wompi_transaction_id: 'wompi-tx-003',
      }),
    });
  });
...
Then('el depósito de garantía se ejecuta como un hold \\(preautorización\\) y no como un cobro definitivo', async ({ page }) => {
